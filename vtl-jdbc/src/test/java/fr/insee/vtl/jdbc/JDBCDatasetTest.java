...
public class JDBCDatasetTest {
  ...
  public void testReadSql() throws ScriptException, SQLException {
    ...
    engine.eval(
        "ds2 := ds1[calc identifier ID := ID, COL4 := if COL3 then COL2 + 1.0 else COL2 - 1.0];");

    var ds2 = (Dataset) bindings.get("ds2");
    assertThat(ds2.getDataAsMap())
        .containsExactly(
            Map.of("ID", 1L, "COL1", "string1", "COL2", 1.2D, "COL3", true, "COL4", 2.2D),
            Map.of("ID", 2L, "COL1", "string2", "COL2", 5.2D, "COL3", false, "COL4", 4.2D),
            Map.of("ID", 3L, "COL1", "string3", "COL2", 3.2D, "COL3", true, "COL4", 4.2D),
            Map.of("ID", 4L, "COL1", "string4", "COL2", 4.2D, "COL3", false, "COL4", 3.2D));
  }
}
