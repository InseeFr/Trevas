...
public class SDMXVTLWorkflow {
  ...
  private static final StaxStructureReaderEngine readerEngineSDMX3 =
  ...
  private Map<String, String> getRulesets() {
    ...
    return vtlRulesetSchemeBean.stream()
        .filter(v -> v.getVtlVersion().equals(engine.getFactory().getLanguageVersion()))
  ...
  public Map<String, String> getTransformations() {
    Set<ITransformationSchemeBean> vtlTransformationSchemeBean =
        sdmxBeans.getVtlTransformationSchemeBean();
    return vtlTransformationSchemeBean.stream()
        .filter(v -> v.getVtlVersion().equals(engine.getFactory().getLanguageVersion()))
        .flatMap(v -> v.getItems().stream())
        .collect(
            Collectors.toMap(
                ITransformationBean::getResult,
                v -> {
        ...
        .map(
            e ->
                new AbstractMap.SimpleEntry<>(
                    e.getKey(),
                    new InMemoryDataset(
                        List.of(), (Map<String, Structured.Component>) e.getValue())))
        .collect(
            Collectors.toMap(
                Map.Entry::getKey,
                Map.Entry::getValue,
  ...
  public Map<String, PersistentDataset> run() {
    ...
    return bindings.entrySet().stream()
        .filter(p -> p.getValue() instanceof PersistentDataset)
