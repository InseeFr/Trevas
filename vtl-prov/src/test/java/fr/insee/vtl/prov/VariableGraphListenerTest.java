...
public class VariableGraphListenerTest {
  ...
  void testSimpleGraph() {
    String expr = "ds2 := ds1;" + "/* test */" + "ds3 <- ds2;" + "ds4 := ds3;" + "ds5 <- ds4;";

    VariableGraphListener provenanceListener = parseAndListen(expr);

    assertThat(provenanceListener.getVariables())
        .map(Variable::toString)
        .containsExactlyInAnyOrder("ds5_0");
    DefaultDirectedGraph<Variable, DefaultEdge> graph = provenanceListener.getGraph();
    assertThat(graph.edgeSet())
        .map(DefaultEdge::toString)
        .containsExactly(
  ...
  void testComposition() {
    String expr = "ds2 := ds1;" + "ds3 <- ds2#foo + 4;";

    VariableGraphListener provenanceListener = parseAndListen(expr);

    assertThat(provenanceListener.getVariables())
        .map(Variable::toString)
        .containsExactlyInAnyOrder("ds3_0");
    DefaultDirectedGraph<Variable, DefaultEdge> graph = provenanceListener.getGraph();
    assertThat(graph.edgeSet())
        .map(DefaultEdge::toString)
        .containsExactly("(ds1_0 : ds2_0)", "(ds2_0 : ds3_0)");
  }

  @Test
  void testArithmeticGraph() {
    String expr = "ds2 := ds1;" + "ds3 <- ds2 + ds1;";
    ...
    assertThat(graph.edgeSet())
        .map(DefaultEdge::toString)
        .containsExactly("(ds1_0 : ds2_0)", "(ds2_0 : ds3_0)", "(ds1_0 : ds3_0)");
  }

  @Test
  void testTwoGraphs() {
    String expr = "ds3 := ds2 + ds1;" + "ds6 := ds5 + ds4;";

    VariableGraphListener provenanceListener = parseAndListen(expr);

    assertThat(provenanceListener.getVariables())
        .map(Variable::toString)
        .containsExactlyInAnyOrder("ds3_0", "ds6_0");
    DefaultDirectedGraph<Variable, DefaultEdge> graph = provenanceListener.getGraph();
    assertThat(graph.edgeSet())
        .map(DefaultEdge::toString)
        .containsExactly(
  ...
  void testTwoConnectedGraphs() {
    String expr = "ds3 := ds2 + ds1;" + "ds5 := ds4 + ds1;";

    VariableGraphListener provenanceListener = parseAndListen(expr);

    assertThat(provenanceListener.getVariables())
        .map(Variable::toString)
        .containsExactlyInAnyOrder("ds5_0", "ds3_0");
    DefaultDirectedGraph<Variable, DefaultEdge> graph = provenanceListener.getGraph();
    assertThat(graph.edgeSet())
        .map(DefaultEdge::toString)
        .containsExactly(
  ...
  void testCyclicGraphs() {
    ...
    String expr = "" + "ds1 := ds0;" + "ds1 := ds1 + 1;" + "ds1 := ds2 * ds1;";

    VariableGraphListener provenanceListener = parseAndListen(expr);
    assertThat(provenanceListener.getVariables())
        .map(Variable::toString)
        .containsExactlyInAnyOrder("ds1_2");
    DefaultDirectedGraph<Variable, DefaultEdge> graph = provenanceListener.getGraph();
    assertThat(graph.edgeSet())
        .map(DefaultEdge::toString)
        .containsExactly(
  ...
  public static void printTree(
      Graph<Variable, DefaultEdge> graph, Variable currentNode, String prefix, boolean isLast) {
