...
Shading runs in the **`compile`** phase (this module has no Java sources) so the modular shaded JAR exists **before** `vtl-parser` compiles in the same reactor `compile` pass. The plain `org.antlr` dependency is marked `optional` so it is not leaked transitively to consumers.
