---
id: index-introduction
title: Trevas
sidebar_label: Overview
slug: /introduction
custom_edit_url: null
pagination_prev: null
---

import useBaseUrl from '@docusaurus/useBaseUrl';
import ThemedImage from '@theme/ThemedImage';
import Video from '@theme/Video';

[![Maven - Trevas parent](https://maven-badges.herokuapp.com/maven-central/fr.insee.trevas/trevas-parent/badge.svg)](https://maven-badges.herokuapp.com/maven-central/fr.insee.trevas/trevas-parent)

[Trevas](https://github.com/InseeFr/Trevas) is a Java library for transforming and validating statistical data using the [VTL 2.1](https://sdmx.org/validation-and-transformation-language-vtl) language.

<Video url={'https://www.youtube.com/watch?v=e8coNevJyXA'} />

## What is VTL?

- Business, logical-level and user-friendly language for statisticians
- Relies on standard structural metadata
- Interoperable, platform-agnostic
- Functional language, enabling data lineage for more insight and reproductibility

## Trevas ecosystem

<div style={{ textAlign: 'center' }}>
	<ThemedImage
		alt="Docusaurus themed image"
		sources={{
			light: useBaseUrl('/img/trevas-ecosystem-light.svg'),
			dark: useBaseUrl('/img/trevas-ecosystem-dark.svg'),
		}}
	/>
</div>

- [Trevas Jupyter](https://github.com/InseeFrLab/Trevas-Jupyter)
- [Trevas Lab](https://github.com/InseeFrLab/Trevas-Lab-UI)
- [Trevas Lab API](https://github.com/InseeFrLab/Trevas-Lab)
- [Trevas Batch](https://github.com/Making-Sense-Info/Trevas-Batch)

## Technical description

Trevas provides VTL engines for different execution contexts, in particular a Java 17 engine and an [Apache Spark](https://spark.apache.org/) engine.

Trevas engines are based on the [JSR 223](https://jcp.org/en/jsr/detail?id=223) specification (Scripting for the Java Platform), which describes the use of scripting languages in Java.

The parser stack (`vtl-antlr` then `vtl-parser`) uses a [relocated ANTLR runtime](/developer-guide/antlr-shaded-runtime) so Trevas and Apache Spark can coexist on the same classpath.
