...
Shading kjører i **`compile`**-fasen (ingen Java-kilder i modulen) slik at den modulære shaded JAR-en finnes **før** `vtl-parser` kompileres i samme reactor-`compile`. Den vanlige `org.antlr`-avhengigheten er merket `optional` for ikke å lekke transitivt til konsumenter.
