...
Le shading s’exécute en phase **`compile`** (ce module n’a pas de sources Java) afin que le JAR modulaire shaded soit disponible **avant** la compilation de `vtl-parser` dans le même passage reactor `compile`. La dépendance `org.antlr` est marquée `optional` pour ne pas fuir transitivement vers les consommateurs.
