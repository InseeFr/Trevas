...
Parquet数据集的元数据是从parquet的架构中推断出来的

#### 数据类型
Trevas引擎负责在Parquet数据类型和Trevas支持的数据类型之间进行转换。

#### 数据角色

VTL的数据角色由Trevas引擎添加到Parquet数据集中，方法是向每个字段描述添加元数据“vtlRole”。
在默认情况下，所有Parquet数据集中没有角色的列将在Trevas中具有角色“MEASURE”。
