...
public class RegisterMethodTest {
    ...
    public void registerTest() throws ScriptException, NoSuchMethodException {
        ...
        engine.registerGlobalMethod("writeCSV", HandleDs.class.getMethod("writeCSV", String.class, fr.insee.vtl.model.Dataset.class));
        engine.registerGlobalMethod("getSize", HandleDs.class.getMethod("getSize", fr.insee.vtl.model.Dataset.class));
        engine.eval("a := loadCSV(\"src/main/resources/ds1.csv\"); " +
                "b := a[calc toto := \"test\"]; " +
                "c := writeCSV(\"src/main/resources/ds1_out.csv\", b);" +
                "d := getSize(b);");
        fr.insee.vtl.model.Dataset a = (fr.insee.vtl.model.Dataset) engine.getContext().getAttribute("a");
    ...
    public static class HandleDs {
        ...
        private static Map<String, fr.insee.vtl.model.Dataset.Role> getRoleMap(Collection<Structured.Component> components) {
        ...
        private static Map<String, fr.insee.vtl.model.Dataset.Role> getRoleMap(fr.insee.vtl.model.Dataset dataset) {
