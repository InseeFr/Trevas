...
public class AggregateTest {
  ...
  public void testAggregateClause() throws ScriptException {
    ...
    assertThat(context.getAttribute("res")).isInstanceOf(Dataset.class);
    assertThat(((Dataset) context.getAttribute("res")).getDataAsMap())
        .containsExactly(
            Map.of(
                "country",
                "france",
    ...
    assertThat(context.getAttribute("res1")).isInstanceOf(Dataset.class);

    var fr = ((Dataset) context.getAttribute("res1")).getDataAsMap().get(0);
    ...
    var no = ((Dataset) context.getAttribute("res1")).getDataAsMap().get(1);

    System.out.println(((Dataset) context.getAttribute("res1")).getDataAsMap());

    assertThat((Double) no.get("stddev_popAge")).isEqualTo(0.0);
    assertThat((Double) no.get("stddev_popWeight")).isEqualTo(0.0);
    // TODO: check when only one value
