...
public class JSONStructureLoader {
  ...
  public static Map<String, List<Structured.Component>> loadStructures(File jsonFile)
      throws Exception {
        ...
        if (componentArray != null && componentArray.isArray()) {
          for (JsonNode componentNode : componentArray) {
            ...
            String componentJson =
                String.format(
                    "{ \"name\": \"%s\", \"type\": \"%s\", \"role\": \"%s\" }", name, type, role);

            Structured.Component component =
                mapper.readValue(componentJson, Structured.Component.class);
  ...
  public static Map<String, Dataset> loadDatasetsFromCSV(File dataStructureFile) throws Exception {
    ...
    if (datasetArray != null && datasetArray.isArray()) {
      for (JsonNode datasetNode : datasetArray) {
        ...
        Dataset dataset =
            new CSVDataset(structure, new FileReader(csvFile), CsvPreference.STANDARD_PREFERENCE);
